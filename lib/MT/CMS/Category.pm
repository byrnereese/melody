    ...
    if ($id) {
        ...
        my $tags_js = MT::Util::to_json(
                             MT::Tag->cache(
                                 blog_id => $blog_id,
                                 class   => (
                                     $app->query->param('_type') eq 'category'
                                     ? 'MT::Category'
                                     : 'MT::Folder'
                                 ),
                                 private => 1
...
sub list {
    ...
    $param{screen_id} = $type eq 'folder' ? 'list-folder' : 'list-category';
...
sub save {
    ...
    if ( my $moved_cat_id = $q->param('move_cat_id') ) {
        $cat = $class->load( $q->param('move_cat_id') ) or return;
        ...
        for my $p ( $q->param ) {
            ...
            $app->run_callbacks( 'cms_pre_save.' . $type,
                                 $app, $cat, $original )
              || return
              $app->errtrans( "Saving [_1] failed: [_2]",
                              $type, $app->errstr );
            ...
            $app->run_callbacks( 'cms_post_save.' . $type,
                                 $app, $cat, $original );
        } ## end for my $p ( $q->param )
    } ## end else [ if ( my $moved_cat_id ...)]

    return $app->errtrans( "The [_1] must be given a name!", $type ) if !$cat;
...
sub category_add {
    ...
    my $data =
      $app->_build_category_list( blog_id => scalar $q->param('blog_id'),
                                  type    => $type );
    my %param;
...
sub category_do_add {
    ...
    $name =~ s/(^\s+|\s+$)//g;
    return $app->errtrans("Category name cannot be blank.") if $name eq '';
    ...
    my $filter_result
      = $app->run_callbacks( 'cms_save_filter.' . $type, $app ) || return;
...
sub js_add_category {
    ...
    return $app->json_result(
                             { id => $obj->id, basename => $obj->basename } );
} ## end sub js_add_category
...
sub save_filter {
    ...
    my ($app) = @_;
    return
      $app->errtrans( "The name '[_1]' is too long!",
...
sub move_category {
    ...
    my $cat = $class->load( $q->param('move_cat_id') ) or return;
    ...
    if ($new_parent_id) {
        my $new_parent = $class->load($new_parent_id) or return;
    ...
    if ( $type eq 'category' ) {    # folder is able to have a same label
        my @siblings = $class->load(
                       { parent => $cat->parent, blog_id => $cat->blog_id } );
        foreach (@siblings) {
            return
              $app->errtrans(
                "The category name '[_1]' conflicts with another category. Top-level categories and sub-categories with the same parent must have unique names.",
