...
sub edit {
    ...
    if ($id) {
        ...
        while ( my $place = $place_iter->() ) {
            ...
            my $entry = $entry_class->load( $place->object_id ) or next;
            my %entry_data = (
                               id    => $place->object_id,
...
sub dialog_list_asset {
    ...
    if ($blog_id) {
        ...
        #
        # In the blog context, the Manage Assets page shouldn't be displayed
        # unless the current user has can_edit_assets or superuser privileges.
        #
        ...
        #
        # In the global context, the Manage Assets page shouldn't be displayed
        # unless the current user has superuser or can_edit_assets privileges
        # on one or more blogs.
        #
        # In order to allow a non-privileged user to upload a userpic within
        # the CMS, we have to make one exception to the rules discussed above.
        # What we're doing is requiring three parameters that are passed to
        # the Upload Asset dialog when a userpic needs to be uploaded.  These
        # parameters limit the usefulness of other pages that the
        # non-privileged user might also be able to access.
        #
    ...
    if ( $filter eq 'class' ) {
        $class_filter = $q->param('filter_val');
    }
    elsif ( $filter eq 'userpic' ) {
        $class_filter = 'image';
        $terms{created_by} = $q->param('filter_val');
    ...
    my $hasher
      = build_asset_hasher( $app, PreviewWidth => 240, PreviewHeight => 240 );
    ...
    foreach my $class ( keys %$classes ) {
        ...
        push @class_loop,
          { class_id => $class, class_label => $classes->{$class}, };
    }
...
sub insert {
    my $app  = shift;
    my $q    = $app->query;
    my $text = $q->param('no_insert') ? "" : _process_post_upload($app);
    return unless defined $text;
    my $file_ext_changes = $q->param('changed_file_ext');
    ...
    my $ctx   = $tmpl->context;
    my $id    = $q->param('id') or return $app->errtrans("Invalid request.");
...
sub asset_userpic {
    ...
    if ( $asset = $param->{asset} ) {
        ...
        $id = $param->{asset_id} || scalar $q->param('id');
...
sub upload_file {
    ...
    complete_insert( $app, asset => $asset, bytes => $bytes, );
}
...
sub complete_insert {
    ...
    if ( !$asset && $q->param('id') ) {
        require MT::Asset;
        $asset = MT::Asset->load( $q->param('id') )
    ...
    my $blog = $asset->blog
      or
      return $app->errtrans( "Can't load blog #[_1].", $q->param('blog_id') );
    my $perms = $app->permissions or return $app->errtrans('No permissions');

    # caller wants asset without any option step, so insert immediately
    if ( $q->param('asset_select') || $q->param('no_insert') ) {
        $q->param( 'id', $asset->id );
        return insert($app);
    }
    ...
    for my $field (
        qw( direct_asset_insert edit_field entry_insert site_path asset_select )
      )
    {
        $param->{$field} = scalar $q->param($field) || '';
    }
    if ( $args{is_image} ) {
        $param->{width}  = $asset->image_width;
        $param->{height} = $asset->image_height;
    }
    if ( $q->param('changed_file_ext') ) {
        my ( $ext_from, $ext_to )
          = split( ",", $q->param('changed_file_ext') );
        $param->{extension_message} =
          $app->translate( "Extension changed from [_1] to [_2]",
                           $ext_from, $ext_to )
          if ( $ext_from && $ext_to );
    }
    if ( !$q->param('asset_select')
...
sub pre_save {
    ...
    if ( defined $tags ) {
        my $blog = $app->blog;
        my $fields
          = $blog ? $blog->smart_replace_fields : MT->config->NwcReplaceField;
...
sub asset_list_filters {
    ...
    return \%filters;
} ## end sub asset_list_filters

sub build_asset_hasher {
    my $app = shift;
    my (%param) = @_;
    my (
         $default_thumb_width,   $default_thumb_height,
...
sub _upload_file {
    ...
    if ( $blog_id = $q->param('blog_id') ) {
        ...
        $base_url
          = $q->param('site_path') ? $blog->site_url : $blog->archive_url;
        $asset_base_url = $q->param('site_path') ? '%r' : '%a';
        ...
        unless ($has_overwrite) {
            ...
            if ( $w_temp && $h_temp && $ext_temp ) {
                ...
                ###
                #
                # The case where there was no file extension wasn't handled
                # properly. We need the ability to append a valid file
                # extension as we bring the file in.
                #
                ###
        ...
        if ( $fmgr->exists($local_file) ) {
            ...
            } ## end else [ if ($has_overwrite) ]
        } ## end if ( $fmgr->exists($local_file...))
    } ## end if ( $blog_id = $q->param...)
    else {
        $blog_id        = 0;
        ...
        my $unique_stem = $stem;
        $local_file
          = File::Spec->catfile( $param{support_path}, $unique_stem . $type );
        my $i = 1;
    ...
    if ( defined($w) && defined($h) ) {
        ...
        $asset_pkg = 'MT::Asset' if $asset_pkg->isa('MT::Asset::Image');
    }
