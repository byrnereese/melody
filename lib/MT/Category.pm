...
__PACKAGE__->install_properties( {
       ...
       defaults   => { parent => 0, allow_pings => 0, },
       class_type => 'category',
       child_of   => 'MT::Blog',
       audit      => 1,
       meta       => 1,
...
sub save {
    ...
    if ( $cat->allow_pings ) {
        ...
        my $blog = MT::Blog->load( $cat->blog_id ) or return;
        my $url = $blog->archive_url;
        $url .= '/' unless $url =~ m!/$!;
        $url .= MT::Util::archive_file_for( undef, $blog, 'Category', $cat );
        $tb->url($url);
        $tb->is_disabled(0);
        $tb->save or return $cat->error( $tb->errstr );
    } ## end if ( $cat->allow_pings)
    else {
...
sub _flattened_category_hierarchy {
    ...
    } ## end foreach my $c (@cats)

    @flattened_cats;
} ## end sub _flattened_category_hierarchy
...
sub children_categories {
    ...
    unless ( $cat->{__children} ) {
        @{ $cat->{__children} } = sort { $a->label cmp $b->label }
          $class->load( { blog_id => $cat->blog_id, parent => $cat->id } );
    }
    @{ $cat->{__children} };
}
