...
sub BEGIN {
    ...
    %functions = (
        ...
        # standard routines
        'core_upgrade_begin' =>
          { code => \&core_upgrade_begin, priority => 1, },
        'core_fix_type'   => { code => \&core_fix_type, priority => 2, },
        'core_add_column' => {
            code => sub {
                shift->core_column_action( 'add', @_ );
            },
            priority => 3,
        },
        'core_drop_column' => {
            code => sub {
                shift->core_column_action( 'drop', @_ );
            },
            priority => 3,
        },
        'core_alter_column' => {
            code => sub {
                shift->core_column_action( 'alter', @_ );
            },
            priority => 3,
        },
        'core_index_column' => {
            code => sub {
                shift->core_column_action( 'index', @_ );
            },
            priority => 3.5,
        },
        'core_seed_database' => { code => \&seed_database, priority => 4, },
        'core_upgrade_templates' =>
          { code => \&upgrade_templates, priority => 5, },
        'core_upgrade_end' => { code => \&core_upgrade_end, priority => 9, },
        'core_finish'      => { code => \&core_finish,      priority => 10, },
    );
} ## end sub BEGIN
    ...
    return {
            ...
            updater       => {
                ...
                sql => [
                    ...
                    'update mt_blog
                      ...
                      where blog_allow_reg_comments is null', 'update mt_blog
        ...
        'core_set_blog_allow_pings' => {
            ...
            updater       => {
                ...
                code      => sub {
                    $_[0]->allow_pings( $_[0]->allow_pings_default );
                },
                sql =>
        ...
        'core_deprecate_old_style_archive_links' => {
            ...
            updater       => {
                ...
                condition => sub {
                    my $blog = shift;
                    my %args = @_;
                    $blog->old_style_archive_links || $args{from} < 3.0;
                },
                code => sub {
                    my ($blog) = @_;
                           ...
                           MT::TemplateMap->load( { blog_id => $blog->id } ) )
                    {
                        ...
                        $map->save;
                    } ## end foreach my $map ( MT::TemplateMap...)
...
} ## end sub core_upgrade_functions
...
sub core_upgrade_meta {
  ...
  TYPE: while ( my ( $registry_type, $reg_class ) = each %$types ) {
        next TYPE
          if $registry_type eq 'plugin' && ref $reg_class;  # plugin reference
    ...
    } ## end while ( my ( $registry_type...))
    return 0;
} ## end sub core_upgrade_meta
...
sub core_upgrade_plugindata_meta_for_table {
    ...
    return 0;
} ## end sub core_upgrade_plugindata_meta_for_table
...
sub core_upgrade_meta_for_table {
    ...
    $sth->execute or return $self->error( $dbh->errstr || $DBI::errstr );
                ...
                if ($metadataref) {
                    ...
                    if ($obj) {
                        foreach my $metaname ( keys %$metadata ) {
                            ...
                            } ## end if ( $metaname eq 'customfields')
                            else {
                                $self->_save_meta( $obj, $metaname,
                                                   $metavalue );
                            }
                        } ## end foreach my $metaname ( keys...)
                    } ## end if ($obj)
                } ## end if ($metadataref)
            } ## end if ( $rawmeta =~ m/^SERG/)
        } ## end if ( defined $rawmeta )
        last if $rows == 100;
    ...
    return $offset;
} ## end sub core_upgrade_meta_for_table
...
sub core_drop_meta_for_table {
    ...
    return 0;
} ## end sub core_drop_meta_for_table
...
sub core_populate_author_auth_type {
    ...
    } ## end else [ if ( $u->type == 1 ) ]
} ## end sub core_populate_author_auth_type
...
sub core_update_password_recover_template {
    ...
    for my $tmpl (@tmpls) {
        ...
        $backup->name(   $backup->name
                       . ' (Backup during upgrade to version '
                       . MT->version_number
                       . ')' );
        $backup->type('backup');
...
} ## end sub core_update_password_recover_template
...
sub migrate_nofollow_settings {
    ...
    return 0;
} ## end sub migrate_nofollow_settings
...
sub update_3x_system_search_templates {
    ...
    0;
} ## end sub update_3x_system_search_templates
...

{
    ...
    } ## end sub _migrate_permission_to_role
    ...
    sub _process_masks {
        ...
        } ## end for my $key ( keys %LegacyPerms)
        my $perm_str
          = scalar(@perms) ? "'" . join( "','", @perms ) . "'" : q();
        $perm->permissions($perm_str);
        $perm->role_mask(0);                ## remove legacy permissions
        $perm;
    } ## end sub _process_masks
    ...
    } ## end sub deprecate_bitmask_permissions
}
...
sub migrate_system_privileges {
    ...
    } ## end while ( my $author = $author_iter...)
} ## end sub migrate_system_privileges
    ...
    if ( !$fix_class ) {
        ...
        foreach my $key ( keys %$class_idx_defs ) {
            ...
            if ( ref($class_idx_def) ) {
                ...
                } ## end else [ if ( !ref $db_idx_def )]
            } ## end if ( ref($class_idx_def...))
            else {
                if ( ref $db_idx_def ) {
                    push @cols_to_index, $key;
                }
            }
        } ## end foreach my $key ( keys %$class_idx_defs)
    } ## end if ( !$fix_class )
...
sub seed_database {
    ...
    $blog->site_path(
          exists $param{blog_path} ? uri_unescape( $param{blog_path} ) : '' );
    $blog->site_url(
            exists $param{blog_url} ? uri_unescape( $param{blog_url} ) : '' );
    $blog->server_offset(
          exists $param{blog_timezone} ? ( $param{blog_timezone} || 0 ) : 0 );
...
sub upgrade_templates {
    ...
    my $installer = sub {
        ...
        my $terms = {};
        $terms->{type}    = $val->{type};
        $terms->{name}    = $val->{name} if $val->{set} ne 'system';
        ...
        if ( $val->{mappings} ) {
            push @arch_tmpl,
              { template => $obj, mappings => $val->{mappings}, };
        }
        return 1;
    };
...
sub rename_php_plugin_filenames {
    ...
    opendir( DIR, $plugin_path ) or return 0;
    ...
    1;
} ## end sub rename_php_plugin_filenames
...
sub post_schema_upgrade {
    ...
    foreach my $fn ( keys %functions ) {
        ...
        $from = $save_from;
    } ## end foreach my $fn ( keys %functions)

    1;
} ## end sub post_schema_upgrade
    ...
    if (@stmts) {
        ...
        eval {
            foreach my $stmt (@stmts)
            {
                ...
                } ## end else [ if ( ref $stmt eq 'CODE')]
                $updated = 1;
            } ## end foreach my $stmt (@stmts)
        };
        if ($@) {
            return $self->error($@);
        }
...
sub core_finish {
    ...
    } ## end foreach my $plugin (@MT::Components)
...
sub core_remove_unique_constraints {
    ...
    1;
} ## end sub core_remove_unique_constraints
...
EOT
    $tmpl->save;
} ## end sub _merge_comment_response_templates_updater
...
sub core_create_config_table {
    ...
    return 1;
} ## end sub core_create_config_table
...
sub core_create_template_maps {
    ...
    1;
} ## end sub core_create_template_maps
...
sub core_update_entry_counts {
    ...
    1;
} ## end sub core_update_entry_counts
...
sub core_update_records {
    ...
    if ( $param{label} ) {
        ...
        $msg
          = $self->translate_escape( $param{message}
                                       || "Updating [_1] records...",
                                     $class_label );
    }
    my $offset = $param{offset};
...
sub mt32_utf8_dirify {
    ...
    $s
      = mt32_xliterate_utf8($s); ## convert two-byte UTF-8 chars to 7bit ASCII
    $s = lc $s;                        ## lower-case.
    $s = MT::Util::remove_html($s);    ## remove HTML tags.
    $s =~ s!&[^;\s]+;!!g;              ## remove HTML entities.
    $s =~ s![^\w\s]!!g;                ## remove non-word/space chars.
