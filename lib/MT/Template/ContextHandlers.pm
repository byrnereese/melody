...
sub _hdlr_app_widget {
    ...
    my $blog_field
      = $blog
      ? qq{<input type="hidden" name="blog_id" value="} . $blog->id . q{" />}
      : "";
...
EOT
} ## end sub _hdlr_app_list_filters
...
EOT
} ## end sub _hdlr_app_page_actions
...
sub _hdlr_get_var {
    my ( $ctx, $args, $cond ) = @_;
    if ( exists( $args->{value} ) && !exists( $args->{op} ) ) {
...
sub _tags_for_blog {
    ...
    if ( !$tag_cache->{$cache_id}{tags} ) {
        require MT::Tag;
        @tags =
          MT::Tag->load_by_datasource( $class->datasource,
                                       { is_private => 0, %$terms },
                                       {%$args} );
...
sub _hdlr_tags {
             ...
             || $args->{exclude_blogs} )
    {
        ...
        } ## end if ( $app->isa('MT::App::Search'...))
    } ## end unless ( $args->{blog_id} ...)
    ...
    if ($needs_entries) {
        foreach my $tag (@$tags) {
            ...
            $tag->{__entries} = delay(
                sub {
                    my @e = MT::Entry->load(@args);
                    \@e;
                }
            );
        } ## end foreach my $tag (@$tags)
    } ## end if ($needs_entries)
...
sub _hdlr_tag_rank {
    ...
    my %term  = $class_type eq 'entry'
      || $class_type eq 'page' ? ( status => MT::Entry::RELEASE() ) : ();
...
sub _hdlr_object_tags {
    ...
    for my $tag (@$tags) {
        ...
        local $ctx->{__stash}{tag_entry_count} = undef
          if ( $obj_type eq 'entry' or $obj_type eq 'page' );
        local $ctx->{__stash}{class_type} = $obj_type
          if !$ctx->{__stash}{class_type};
        defined( my $out = $builder->build( $ctx, $tokens, $cond ) )
          or return $ctx->error( $builder->errstr );
        $res .= $glue if defined $glue && length($res) && length($out);
        $res .= $out;
...
{
    my %include_stack;
    my %restricted_include_filenames = ( 'config.cgi' => 1, 'passwd' => 1 );
...
=cut
    ...
    sub _hdlr_include {
        ...
        my $out
          ...
          :   $ctx->error( MT->translate('No template to include specified') );

        return $out;
    } ## end sub _hdlr_include
    ...
    sub _include_module {
        ...
        if ($use_ssi) {
            ...
            if ( $ctx->var('system_template') ) {
                ...
                my $extra_path
                  ...
                  || $arg->{key} : $tmpl->cache_path ? $tmpl->cache_path : '';
                %include_recipe = (
                                    name => $tmpl_name,
                                    id   => $tmpl->id,
                                    path => $extra_path,
        ...
        if ( $cache_expire_type == 2 ) {
            ...
            if (@types) {
                ...
                } ## end if ( my $latest = MT::Touch...)
            } ## end if (@types)
        } ## end if ( $cache_expire_type...)
...
sub _hdlr_link {
    ...
    if ( my $tmpl_name = $arg->{template} ) {
        ...
        my $tmpl = MT::Template->load( {
          ...
          || MT::Template->load(
                { name => $tmpl_name, type => 'index', blog_id => $blog_id } )
          || MT::Template->load(
             { outfile => $tmpl_name, type => 'index', blog_id => $blog_id } )
          or return $ctx->error(
...
sub _hdlr_set_var {
    ...
    if ( defined $key ) {
        ...
        if ( ( defined $func ) && ( 'delete' eq lc($func) ) ) {
            delete $data->{$key};
        }
        else {
            $data->{$key} = $val;
...
sub _hdlr_author_has_entry {
    my ($ctx) = @_;
    my $author = $ctx->stash('author') or return $ctx->_no_author_error();
...
sub _hdlr_authors {
    ...
    if ( my $status_arg = $args->{status} || 'enabled' ) {
        ...
        my $status
          = [ { name => 'enabled', id => 1 }, { name => 'disabled', id => 2 },
          ];
    ...
    if ( $role_arg = $args->{role} || $args->{roles} ) {
        ...
        if ($cexpr) {
            ...
            for my $role (@$roles) {
                my $iter =
                  MT::Association->load_iter(
                    { role_id => $role->id, %blog_terms },
                    \%blog_args );
                while ( my $as = $iter->() ) {
    ...
    if ( $args->{namespace} ) {
        ...
        } ## end else [ if ( my $scoring_to = ...)]
    } ## end if ( $args->{namespace...})
  ...
  AUTHOR: while ($next) {
    ...
    if ( $re_sort && ( scalar @authors ) ) {
        ...
        if ( 'score' eq $col ) {
            ...
            @authors = @tmp;
        } ## end elsif ( 'rate' eq $col )
    } ## end if ( $re_sort && ( scalar...))
...
sub _hdlr_author_auth_icon_url {
    ...
    return $url;
} ## end sub _hdlr_author_auth_icon_url
...
sub _hdlr_entries {
    ...
    if ( $ctx->stash('results') && ($args->{search_results} || 0) == 1 ) {
    ...
    if ( my $category_arg = $args->{category} || $args->{categories} ) {
        ...
        if ( ref $category_arg ) {
            ...
            if (
                 ( $category_arg !~ m/\b(AND|OR|NOT)\b|[(|&]/i )
                 && ( (
    ...
    if ( my $tag_arg = $args->{tags} || $args->{tag} ) {
        ...
        if ($cexpr) {
            ...
            my $preloader = sub {
                ...
                my $args
                  = { %blog_args, fetchonly => ['tag_id'], no_triggers => 1 };
        ...
        if ($need_join) {
            ...
            $args{join} = MT->model('objectscore')->join_on(
                  undef,
                  {
                    ...
                    (
                          !$entries
                        && $scored_by ? ( author_id => $scored_by->id ) : ()
                    ),
                  },
                  { unique => 1, }
            );
...
sub _rco_entries_iter {
    ...
    my @entries;
    delete $entry_args->{direction} if exists $entry_args->{direction};
    delete $entry_args->{sort}      if exists $entry_args->{sort};

    my $rco_iter = sub {
        if ( !@entries ) {
            require MT::Comment;
            my $iter =
              MT::Comment->max_group_by(
                      { visible => 1, %$blog_terms, },
                      {
                        join =>
                          MT::Entry->join_on(
                             undef,
                             { 'id' => \'=comment_entry_id', %$entry_terms, },
                             {%$entry_args}
                          ),
                        %$blog_args,
                        group  => ['entry_id'],
...
sub _hdlr_entry_author_link {
    ...
    return $displayname;
} ## end sub _hdlr_entry_author_link
...
sub _hdlr_entry_categories {
    ...
    $res;
} ## end sub _hdlr_entry_categories
...
} ## end sub _hdlr_remote_sign_in_link
...
sub _hdlr_remote_sign_out_link {
    ...
    "$path$comment_script?__mode=handle_sign_in$static_arg&amp;logout=1"
      . ( $e ? "&amp;entry_id=" . $e->id : '' );
} ## end sub _hdlr_remote_sign_out_link
...
sub _hdlr_sys_date {
    ...
    unless ( $args->{ts} ) {
        ...
        $args->{ts} = sprintf "%04d%02d%02d%02d%02d%02d", $ts[5] + 1900,
          $ts[4] + 1, @ts[ 3, 2, 1, 0 ];
    }
    return _hdlr_date( $ctx, $args );
}
    ...
    for my $c (@comments) {
        ...
        if ( $c->commenter_id ) {
            $ctx->stash(
                         'commenter',
                         delay(
                                sub { MT::Author->load( $c->commenter_id ) }
                         )
            );
        }
        else {
        ...
        $html .= $out;
        $i++;
    } ## end for my $c (@comments)
    if ( !@comments ) {
        return _hdlr_pass_tokens_else(@_);
    }
    return $html;
} ## end sub _hdlr_comments
...
sub _hdlr_comment_author_link {
    ...
    if ($cmntr) {
        ...
        if ( $cmntr->url ) {
            return
              sprintf( qq(<a title="%s" href="%s"%s>%s</a>),
    ...
    return $name;
} ## end sub _hdlr_comment_author_link
...
sub _hdlr_comment_author_identity {
    my ( $ctx, $args ) = @_;
    my $cmt = $ctx->stash('comment') or return $ctx->_no_comment_error();
    my $cmntr = $ctx->stash('commenter');
    unless ($cmntr) {
        if ( $cmt->commenter_id ) {
            $cmntr = MT::Author->load( $cmt->commenter_id ) or return "?";
        }
        else {
            return q();
        }
    ...
    return $result;
} ## end sub _hdlr_comment_author_identity
...
sub _hdlr_comment_entry {
    my ( $ctx, $args, $cond ) = @_;
    my $c = $ctx->stash('comment') or return $ctx->_no_comment_error();
    my $entry = MT::Entry->load( $c->entry_id ) or return '';
    local $ctx->{__stash}{entry} = $entry;
    local $ctx->{current_timestamp} = $entry->authored_on;
...
sub _hdlr_comment_replies_recurse {
    ...
    $html;
} ## end sub _hdlr_comment_replies_recurse
...
} ## end sub _hdlr_commenter_name_thunk
...
sub _hdlr_user_session_cookie_domain {
    ...
    return $blog_domain;
} ## end sub _hdlr_user_session_cookie_domain
...
sub _hdlr_commenter_isauthor {
    ...
    return 0;
} ## end sub _hdlr_commenter_isauthor
...
sub _hdlr_archive_prev_next {
    ...
    if ( $arctype->date_based && $arctype->category_based ) {
        ...
        $entry
          ...
          ? $arctype->previous_archive_entry($param)
          : $arctype->next_archive_entry($param);
    } ## end else [ if ( $arctype->date_based...)]
    if ($entry) {
        my $builder = $ctx->stash('builder');
    ...
    $res;
} ## end sub _hdlr_archive_prev_next
...
sub _hdlr_index_list {
    ...
    my $builder = $ctx->stash('builder');
    my $iter
      = MT::Template->load_iter(
                     { type   => 'index', blog_id => $ctx->stash('blog_id') },
                     { 'sort' => 'name' } );
    my $res = '';
    while ( my $tmpl = $iter->() ) {
...
sub _hdlr_archives {
    ...
    if ( ( $tmpl && $tmpl->type eq 'archive' ) && $archiver->date_based ) {
...
sub _hdlr_archive_title {
    ...
    if ( $archiver->entry_based ) {
        ...
        } ## end else [ if ( $entries && ref($entries...))]
    } ## end if ( $archiver->entry_based)
    my $title
      = ( @entries && $entries[0] )
      ? $archiver->archive_title( $ctx, $entries[0] )
      : $archiver->archive_title( $ctx, $ctx->{current_timestamp} );
    defined $title ? $title : '';
} ## end sub _hdlr_archive_title
...
sub _hdlr_calendar {
    ...
    if ( $prefix = lc( $args->{month} || '' ) ) {
        if ( $prefix eq 'this' ) {
            ...
            return $ctx->error(
              ...
              unless length($prefix) == 6;
        }
    } ## end if ( $prefix = lc( $args...))
    else {
        $prefix = $today;
    }
    my ( $cat_name, $cat );
    if ( $cat_name = $args->{category} ) {
        $cat
          = MT::Category->load( { label => $cat_name, blog_id => $blog_id } )
          or return $ctx->error(
        ...
        unless ($is_padding) {
            ...
            unless ( $no_loop || $iter_drained ) {
                while ( my $entry = $iter->() ) {
                    ...
                    push( @left, $entry ), last unless $e_day eq $this_day;
                    push @entries, $entry;
                }
...
sub _hdlr_categories {
    ...
    my %counts;
    my $count_tag
      = $class_type eq 'category' ? 'CategoryCount' : 'FolderCount';
    my $uncompiled = $ctx->stash('uncompiled') || '';
    my $count_all = 0;
    ...
    my $entry_count_of = sub {
        my $cat = shift;
        return delay( sub { $cat->entry_count } ) unless $count_all;
    ...
    # This block looks for the tag or tags attribute and then attempts
    # to build a tag filter subroutine for the specified value
    my $tag_filter = undef;
    if ( my $tag_arg = $args->{tags} || $args->{tag} ) {
        ...
        if ($cexpr) {
            ...
            # This attempts to load MT::ObjectTags based on a scalar value,
            # entry_id, that is passed in and it effectively serves as the
            # filter for the tag filter.
            my $preloader = sub {
                my ($entry_id) = @_;
                my $cterms = {
                               tag_id            => \@tag_ids,
                ...
                my $cargs = { fetchonly => ['tag_id'], no_triggers => 1, };

                # Here we attempt to load some MT::ObjectTag objects, but
                # pull back only the tag_id attribute to reduce the
                # memory footprint
    ...
    # If there is a tag filter, we need to test $cat,
    # otherwise a bad category could go through.
    ...
    while ( defined($cat) ) {
        ...
        my $next_cat = $iter->();
        $next_cat = $iter->()
          if ( !$next_cat or ( $tag_filter and !$tag_filter->($next_cat) ) );
        if ( !$args->{show_empty} ) {
        ...
        last if $last;
        $cat = $next_cat;
    } ## end while ( defined($cat) )
    $res;
} ## end sub _hdlr_categories
...
sub _hdlr_category_basename {
    ...
    return $basename;
} ## end sub _hdlr_category_basename
...
sub _hdlr_category_comment_count {
    ...
    return $ctx->count_format( $count, $args );
} ## end sub _hdlr_category_comment_count
...
sub _hdlr_category_archive {
    ...
    $arch = MT::Util::strip_index( $arch, $blog ) unless $args->{with_index};
    $arch;
} ## end sub _hdlr_category_archive
...
sub _hdlr_category_tb_link {
    ...
    if ( !$cat ) {
        ...
        $cat
          = MT::Category->load(
                   { label => $cat_name, blog_id => $ctx->stash('blog_id') } )
          or return $ctx->error("No such category '$cat_name'");
    }
    require MT::Trackback;
    my $tb = MT::Trackback->load( { category_id => $cat->id } ) or return '';
    my $cfg = $ctx->{config};
    my $path = _hdlr_cgi_path($ctx);
    return $path . $cfg->TrackbackScript . '/' . $tb->id;
} ## end sub _hdlr_category_tb_link
...
sub _hdlr_category_prevnext {
    ...
    return '';
} ## end sub _hdlr_category_prevnext
...
sub _hdlr_ping_date {
    my ( $ctx, $args ) = @_;
    my $p = $ctx->stash('ping') or return $ctx->_no_ping_error();
...
sub _hdlr_ping_id {
    my ($ctx) = @_;
    my $ping = $ctx->stash('ping') or return $ctx->_no_ping_error();
    return $ping->id;
}
...
} ## end sub _hdlr_if_comments_allowed
...
} ## end sub _hdlr_if_comments_active
...
sub _hdlr_sub_cats_recurse {
    ...
    while ( my $c = shift @$cats ) {
        ...
        $res .= $out;
        $count++;
    } ## end while ( my $c = shift @$cats)

    $res;
} ## end sub _hdlr_sub_cats_recurse
...
sub _hdlr_sub_categories {
    ...
    while ( my $cat = shift @$cats ) {
        ...
        $res .= $out;
        $count++;
    } ## end while ( my $cat = shift @$cats)

    $res;
} ## end sub _hdlr_sub_categories
...
sub _hdlr_parent_categories {
    ...
    $res;
} ## end sub _hdlr_parent_categories
...
sub _hdlr_top_level_parent {
    ...
    $out;
} ## end sub _hdlr_top_level_parent
...
sub cat_path_to_category {
    ...
    my @cats = $class->load( { label => $top, parent => 0, %blog_terms },
                             \%blog_args );
    if (@cats) {
    ...
    if ( !@cats && $path ) {
        @cats
          = ( $class->load( { label => $path, %blog_terms, }, \%blog_args ) );
    }
    @cats;
} ## end sub cat_path_to_category
...
sub _hdlr_entries_with_sub_categories {
    ...
    $res;
} ## end sub _hdlr_entries_with_sub_categories
...
} ## end sub _hdlr_sub_category_path
...
sub _hdlr_is_ancestor {
    ...
    my $iter
      = MT::Category->load_iter(
                          { blog_id => $blog_id, label => $args->{'child'} } )
      || undef;
    while ( my $child = $iter->() ) {
...
sub _hdlr_is_descendant {
    ...
    my $iter = MT::Category->load_iter(
                        { blog_id => $blog_id, label => $args->{'parent'} } );
...
sub _hdlr_entry_edit_link {
    ...
    return sprintf q([<a href="%s%s%s">%s</a>]), $url, $cfg->AdminScript,
      $app->uri_params(
         'mode' => 'view',
         args =>
           { '_type' => $entry->class, id => $entry->id, blog_id => $blog_id }
      ), $edit_text;
} ## end sub _hdlr_entry_edit_link
...
sub _hdlr_asset_thumbnail_link {
    ...
    $ret .= sprintf qq(><img src="%s" width="%d" height="%d" alt="" /></a>),
      $url, $w, $h;
    $ret;
} ## end sub _hdlr_asset_thumbnail_link
...
sub _check_page {
    my ($ctx) = @_;
    my $e = $ctx->stash('entry') or return $ctx->_no_page_error();
    return $ctx->_no_page_error() if ref $e ne 'MT::Page';
    1;
}
...
sub _hdlr_author_next_prev {
    ...
    $res;
} ## end sub _hdlr_author_next_prev
...
sub _hdlr_section {
    ...
    if ( defined $timeout && ( $timeout > 0 ) ) {
        if ( defined $cache_id ) {
            ...
            if ( $args->{by_user} ) {
                my $author = $app->user
                  or return $ctx->error( MT->translate("Can't load user.") );
    ...
    if ( $cache_require && ( defined $cache_id ) ) {
        ...
        $sess = MT::Session->new;
        $sess->set_values(
             { id => $cache_id, kind => 'CO', start => time, data => $out } );
        $sess->save();
    }
