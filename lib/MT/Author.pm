...
sub set_commenter_perm {
    ...
    my %perm_spec = ( author_id => $this->id(), blog_id => $blog_id );
...
sub save {
    ...
    if ( defined $privs ) {
        ...
        $perm->save
          or
          return $auth->error( "Error saving permission: " . $perm->errstr );
    }
    1;
} ## end sub save
...
sub can_view_log {
    ...
    if (@_) {
        ...
        $author->is_superuser() || $author->permissions(0)->can_view_log(@_);
    }
}
...
sub group_count {
    ...
    MT::Association->count(
          { type => MT::Association::USER_GROUP(), author_id => $author->id, }
    );
}
...
} ## end sub userpic_thumbnail_options
...
sub userpic_file {
    ...
    my $thumb_file =
      File::Spec->catfile( $asset->thumbnail_path(%thumb_param),
