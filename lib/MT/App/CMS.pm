...
sub core_methods {
    ...
    return {
        ...
        'logout' =>
          { code => sub { $_[0]->SUPER::logout(@_) }, requires_login => 0, },
        'start_recover' =>
          { code => "${pkg}Tools::start_recover", requires_login => 0, },
        'recover' =>
          { code => "${pkg}Tools::recover_password", requires_login => 0, },
        'new_pw' =>
...
#        'save_cfg_system_users'   => "${pkg}User::save_cfg_system_users",
        ...
        'upgrade' => { code => "${pkg}Tools::upgrade", requires_login => 0, },
...
sub init_request {
    ...
    unless ( defined $app->{upgrade_required} ) {
        ...
        } ## end if ( ( $mode ne 'logout'...))
    } ## end unless ( defined $app->{upgrade_required...})
            ...
            'open_batch_editor' => {
                ...
                condition => sub {
                    ...
                    $q->param('blog_id')
                      ...
                      && !( defined $q->param('filter_val')
                            && $q->param('filter_val') == MT::Entry::JUNK() )
                      && !( defined $q->param('filter_key')
                            && $q->param('filter_key') eq 'spam_entries' );
                },
            },
        },
        'page' => {
...
sub core_list_filters {
    ...
    return {
            ...
            _by_date => {
                label => sub {
                    ...
                    if ( $from && $to ) {
                        ...
                        return
                          $app->translate( "[_1] posted since [_2]",
                                           $label, $from );
                    }
                    elsif ($to) {
                        return
                          $app->translate( "[_1] posted on or before [_2]",
                                           $label, $to );
                    }
                },
                handler => sub {
        ...
        comment => {
            ...
            _comments_by_entry => {
                label => sub {
                    ...
                    return
                      MT->translate( "All comments for [_1] '[_2]'",
            ...
            _by_date => {
                label => sub {
                    ...
                    if ( $from && $to ) {
                        ...
                        return
                          $app->translate( "Comments posted since [_1]",
                                           $from );
                    }
                    elsif ($to) {
                        return
        ...
        tag => {
                 entry => { label => 'Tags with entries', order => 100, },
                 page  => { label => 'Tags with pages',   order => 200, },
                 asset => { label => 'Tags with assets',  order => 300, },
        },
        sys_user => {
            enabled => {
                label   => 'Enabled Users',
...
} ## end sub core_list_filters

sub core_menus {
    my $app = shift;
    my $q   = $app->query;
    return {
        'create' => { label => "Create",      order => 100, },
        'manage' => { label => "Manage",      order => 200, },
        'design' => { label => "Design",      order => 300, },
        'prefs'  => { label => "Preferences", order => 400, },
        'tools'  => { label => "Tools",       order => 500, },

        'create:blog' => { label => "Blog", order => 100, view => "system", },
        'create:blog' => {
                           label      => "Blog",
                           order      => 200,
                           view       => "system",
        ...
        'manage:comment' => {
            ...
            condition => sub {
                ...
                if ( $q->param('blog_id') ) {
                    my $perms
                      = $app->user->permissions( $q->param('blog_id') );
                    return 1 if $perms && $perms->can_view_feedback;
                }
                else {
                    require MT::Permission;
                    my @blogs
        ...
        'tools:search' =>
          { label => 'Search', order => 1000, mode => 'search_replace' },
...
sub init_core_callbacks {
    ...
    $app->_register_core_callbacks( {
            ...
            # associations
            $pkg . 'save_permission_filter.association' => sub {
            ...
            $pkg
              . 'delete_permission_filter.author' => "${pfx}User::can_delete",
            ...
            $pkg
              . 'view_permission_filter.comment' => "${pfx}Comment::can_view",
            $pkg
              . 'save_permission_filter.comment' => "${pfx}Comment::can_save",
            $pkg
            ...
            $pkg
              . 'delete_permission_filter.entry' => "${pfx}Entry::can_delete",
            ...
            $pkg
              . 'view_permission_filter.ping' => "${pfx}TrackBack::can_view",
            $pkg
              . 'save_permission_filter.ping' => "${pfx}TrackBack::can_save",
            $pkg
            ...
            # tags
            $pkg . 'save_permission_filter.tag' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            ...
            # log
            $pkg . 'save_permission_filter.log' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.log' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # config
            $pkg . 'save_permission_filter.config' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.config' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # fileinfo
            $pkg . 'save_permission_filter.fileinfo' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.fileinfo' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # objectasset
            $pkg . 'save_permission_filter.objectasset' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.objectasset' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # objectscore
            $pkg . 'save_permission_filter.objectscore' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.objectscore' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # objecttag
            $pkg . 'save_permission_filter.objecttag' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.objecttag' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # permission
            $pkg . 'save_permission_filter.permission' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.permission' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # plaement
            $pkg . 'save_permission_filter.placement' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.placement' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # session
            $pkg . 'save_permission_filter.session' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.session' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # templatemap
            $pkg . 'save_permission_filter.templatemap' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.templatemap' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # touch
            $pkg . 'save_permission_filter.touch' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.touch' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # trackback
            $pkg . 'save_permission_filter.trackback' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.trackback' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # ts_error
            $pkg . 'save_permission_filter.ts_error' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.ts_error' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # ts_exitstatus
            $pkg . 'save_permission_filter.ts_exitstatus' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.ts_exitstatus' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # ts_funcmap
            $pkg . 'save_permission_filter.ts_funcmap' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.ts_funcmap' => sub {
                $app->error( $app->translate("Invalid request.") );
            },

            # ts_job
            $pkg . 'save_permission_filter.ts_job' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
            $pkg . 'delete_permission_filter.ts_job' => sub {
                $app->error( $app->translate("Invalid request.") );
            },
...
sub build_blog_selector {
    ...
    if (@blogs) {
        ...
        for my $blog (@blogs) {
            ...
            push @data,
              { top_blog_id => $blog->id, top_blog_name => $blog->name };
...
sub build_menus {
    ...
    foreach my $id (@top_ids) {
        ...
        foreach my $sub_id (@sub_ids) {
            my $sub = $menus->{$sub_id};
            next if $sub->{view} && ( $sub->{view} ne $view );
            $sub->{'id'} = $sub_id;
...
sub show_error {
    ...
    if ( $method_info->{app_mode} && $method_info->{app_mode} eq 'JSON' ) {
        ...
        if ( !exists( $param->{goback} ) && exists( $app->{goback} ) ) {
            ...
            my $url = $app->uri( mode => 'rebuild_confirm',
...
sub load_default_entry_prefs {
    ...
    } ## end else [ if ( $perm && $perm->entry_prefs)]
    $prefs ||= 'Default|Bottom';
    return $prefs;
} ## end sub load_default_entry_prefs
...
sub _convert_word_chars {
    ...
    my $blog = $app->blog;
    my $smart_replace
      = $blog ? $blog->smart_replace : MT->config->NwcSmartReplace;
...
sub add_to_favorite_blogs {
    ...
    unshift @current, $fav;
    @current = @current[ 0 .. 9 ] if @current > 10;
...
sub _entry_prefs_from_params {
    ...
    $prefs;
} ## end sub _entry_prefs_from_params
...
sub rebuild_these {
    ...
    if ( exists $options{how} && ( $options{how} eq NEW_PHASE ) ) {
        ...
        my %param = (
                      is_full_screen => 1,
                      redirect_target =>
                        $app->uri( mode => 'rebuild_phase', args => $params )
        );
        ...
        return $app->load_tmpl( 'rebuilding.tmpl', \%param );
    } ## end else [ if ( exists $options{how...})]
} ## end sub rebuild_these
...
sub remove_preview_file {
    ...
    if ( my $preview = $q->param('_preview_file') ) {
        require MT::Session;
        if ( my $tf = MT::Session->load( { id => $preview, kind => 'TF', } ) )
        {
            my $file = $tf->name;
...
sub _build_category_list {
    ...
    while ( my $obj = shift @cats ) {
        ...
        $row->{is_object}            = 1;
        push @data, $row;
    } ## end while ( my $obj = shift @cats)
    \@data;
} ## end sub _build_category_list
